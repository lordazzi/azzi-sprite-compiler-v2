...
{
    class SprDecompiler70 : SprDecompiler
    {
        private UInt16 spritesAmount;

        public SprDecompiler70(TibiaFile spr)
        {
            if (File.Exists(spr.getFile()))
            {
                //lendo assinatura do client
                ...
                spritesAmount = readspr.ReadUInt16();
                this.SprBmp = new Bitmap[spritesAmount + 1];//array de sprites
                this.offset = new UInt32[spritesAmount];//array de deslocamentos
                ...
                for (int i = 1; i <= spritesAmount; i++)
                {
                    ...
                    readspr.BaseStream.Seek(offset[i], SeekOrigin.Begin);
